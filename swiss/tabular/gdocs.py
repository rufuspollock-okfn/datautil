...
class GDocsReaderTextDb(ReaderBase):
    ...
    def read(self, sheet_name='Sheet1'):
        ...
        db = dbs[0]
        tab = db.GetTables(name=sheet_name)[0]
        tdata = TabularData()
        tab.LookupFields()
        tdata.header = tab.fields
        rows = tab.GetRecords(1,20)
...
# textdb is nicer but Spreadsheet allows one to get all cells (even when blank
# lines)
